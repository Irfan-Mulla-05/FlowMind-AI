...
4. Run the backend with `npm run dev` on `http://localhost:5000`
5. Run the frontend with `npm run dev` on `http://localhost:5173`
...
### Render Root Build Error Fix

If Render shows:

```text
Could not read package.json: ENOENT /opt/render/project/src/package.json
```

it means the service is building from the repository root instead of the `backend` folder.

You now have two valid fixes in this repo:

1. Preferred: set the Render service Root Directory to `backend`
2. Alternative: deploy from repo root using the root `package.json`, which forwards startup to `backend`

This repo also includes `render.yaml` so you can create the backend and frontend services as a Render Blueprint with the correct directories already defined.
