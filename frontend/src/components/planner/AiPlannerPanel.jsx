...
const AiPlannerPanel = ({ isWorking = false, onPlan, onReschedule, onBreakdown, onVoice }) => {
  ...
  return (
    <div className="space-y-4">
      <div className="glass-panel rounded-[28px] p-5">
        ...
        />
        <Button className="mt-3" onClick={() => onBreakdown(goal)} disabled={isWorking}>
          {isWorking ? "Working..." : "Break down goal"}
        </Button>
      </div>
      ...
      <div className="glass-panel rounded-[28px] p-5">
        ...
        <textarea
          ...
          className="mt-3 w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-white outline-none"
        />
        <Button className="mt-3" onClick={() => onVoice(transcript)} disabled={isWorking}>
          {isWorking ? "Working..." : "Convert to tasks"}
        </Button>
      </div>

      <div className="grid gap-3 md:grid-cols-2">
        <Button variant="secondary" onClick={onPlan} disabled={isWorking}>
          Generate day plan
        </Button>
        <Button variant="secondary" onClick={onReschedule} disabled={isWorking}>
